...
namespace Tests.UpdaterTests
{
	public class UpdaterAppTests
	{
		...
		public void Main_InvalidArguments_ShowsErrorMessage()
		{
			...
			// Act
			UpdaterApp.Program.CallMain(args);
		...
		// FIX: Failina leidziant kartu su visais testais.
		[Fact]
		public void Main_ExtractsAndReplacesFilesCorrectly()
		{
			...
			string[] args = ["9999999", "mainApp", Path.Combine(tempDirectoryManager.TempDirectory, "UpdaterApp.exe"), testZipPath];
		...
		public async Task Main_SendsShutdownSignalToNamedPipeServer()
		{
			...
			Task programTask = Task.Run(() => { UpdaterApp.Program.CallMain(args, cts.Token); });
			Task checkTask = Task.Run(() => { while (!shutdownSignalReceived) { } cts.Cancel(); });
			await Task.WhenAny(programTask, checkTask); // Ensure the test doesn't hang indefinitely
